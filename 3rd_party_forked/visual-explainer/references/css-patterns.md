...
Use this pattern when the page is not just explanatory, but also a place where the user should answer inside the HTML itself: decision workbooks, questionnaires, sign-off pages, structured review sheets, or requirement-gathering pages.

**Important:** the question structure should match the content. Do **not** force every prompt into a single template.

Common models:
- **single-select** — exactly one option is valid
- **multi-select** — several conditions can be true at once
- **text-only** — presets would create fake precision
- **mixed preset + notes** — canonical choices exist, but the user may also need nuance

Design rules:
- If the page has multiple prompts, include a bottom summary/export area.
- If presets and notes can both exist, the export should combine them.
- If the page is substantial, include a per-question clear/reset control and a global clear action.
- `localStorage` is a good default unless the page should be ephemeral.
- For local `file://` pages, clipboard needs `navigator.clipboard` **and** `document.execCommand('copy')` fallback.
- Interactive choice cards should be real `<button type="button">` elements — not clickable `<div>`s.
...
.choice-card.is-selected {
  ...
  border-color: color-mix(in srgb, var(--border-bright) 35%, var(--accent) 65%);
}
...
```html
<button type="button" class="choice-card" data-question="q1" data-choice="A">
  <div class="choice-card__badge">A</div>
  <h3>Keep current behavior</h3>
  <p>Use when the current contract is already correct.</p>
</button>
```

### Notes + Per-Question Clear Pattern
...
<div class="question-tools">
  <div>
    <h2>Decision 1 — What should this control mean?</h2>
    <p>Choose a preset, write notes, or do both.</p>
  </div>
  <button type="button" class="clear-question" data-clear-question="q1">Clear question</button>
</div>

<div class="selection-preview" id="preview-q1">No preset selected yet.</div>
<textarea class="answer-notes" data-question-notes="q1"></textarea>
```
...
<div class="summary-panel">
  <div class="summary-tools">
    ...
    <span id="copyStatus">Nothing copied yet.</span>
  </div>
  <textarea id="summaryOutput" class="summary-output" readonly></textarea>
</div>
```

### State + Copy Logic Pattern

You do not need a framework for this. A small inline script is enough.

```js
const STORAGE_PREFIX = 've-workbook::';
...
**String safety rule:** when generating JS inside HTML, never accidentally break a quoted string across lines. For multiline summary output, use explicit `\n`, arrays + `join('\n')`, or `JSON.stringify(...)` when embedding structured data.

**Validation rule:** if your page includes custom JS, syntax-check it before delivery (for example, by extracting the script and running `node --check`). Then open the page and verify that the interactive controls really work.

See `../templates/interactive-workbook.html` for a full working example with:
- single-select + notes
- text-only prompts
- multi-select + notes
- live summary generation
- localStorage persistence
- copy fallback for local HTML files
