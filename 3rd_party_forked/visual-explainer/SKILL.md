...
description: Generate beautiful, self-contained HTML pages that visually explain systems, code changes, plans, data, and structured decisions. Use when the user asks for a diagram, architecture overview, diff review, plan review, project recap, comparison table, interactive workbook, or any visual explanation of technical concepts. Also use proactively when you are about to render a complex ASCII table (4+ rows or 3+ columns) — present it as a styled HTML page instead.
license: MIT
...
Generate self-contained HTML files for technical diagrams, visualizations, data tables, and interactive workbook-style pages. Always open the result in the browser. Never fall back to ASCII art when this skill is loaded.
...
| `interactive-workbook` | Generate an interactive HTML workbook for decisions, questionnaires, approvals, or structured answer capture |
...
**What type of content?** Architecture, flowchart, sequence, data flow, schema/ER, state machine, mind map, class diagram, C4 architecture, data table, interactive workbook/questionnaire, timeline, dashboard, or prose-first page. Each has distinct layout needs and rendering approaches (see Diagram Types below).
...
- For interactive reviews, questionnaires, decision workbooks, sign-off pages, or any HTML page where the user will answer inside the browser: read `./templates/interactive-workbook.html` and the "Interactive Workbook Pages" section in `./references/css-patterns.md`
...
| Interactive workbook / questionnaire | HTML form controls + card layout + summary/export panel | The user needs to choose answers, type notes, and copy/share a structured result |
...
### Interactive Workbooks / Questionnaires / Decision Pages
Use this pattern when the page is not just explanatory, but also a place where the user should answer inside the browser.

Use for:
- decision workbooks
- questionnaires
- sign-off / approval pages
- structured review sheets
- requirement-gathering pages where the user should choose options and add rationale

**First design the question model.** Do not force every prompt into A/B/C. Some prompts should be:
- **single-select** — exactly one option
- **multi-select** — several conditions can be true
- **text-only** — presets would create fake precision
- **mixed preset + notes** — canonical choices exist, but the user may need nuance

**If presets exist, allow notes to coexist.** If the user selects a preset and also types notes, the export summary should combine both.

**Separate locked assumptions from open questions** when the source material naturally supports that split. It reduces noise and makes the page easier to use.

**Always include an export surface** when the workbook has multiple prompts. A bottom summary panel should aggregate answers into plain text or markdown that the user can copy back into chat or a doc.

**Persistence default:** use `localStorage` unless the user explicitly wants an ephemeral page.

**Clipboard default:** for `file://` pages, implement both `navigator.clipboard.writeText(...)` and a `document.execCommand('copy')` fallback, with manual selection as the last resort.

**Control semantics:** interactive option cards should be real `<button type="button">` elements, not clickable `<div>`s.

See `./templates/interactive-workbook.html` and the "Interactive Workbook Pages" section in `./references/css-patterns.md` for the reference pattern.
...
**Usage:**
```bash
bash /Users/luketych/.pi/agent/skills/visual-explainer/scripts/share.sh <html-file>
```

**Example:**
```bash
bash /Users/luketych/.pi/agent/skills/visual-explainer/scripts/share.sh ~/.agent/diagrams/my-diagram.html
...
- **Interactive behavior works**: If the page has custom controls, verify they actually click/toggle, per-question clear/reset works, and any summary/export area updates live.
- **Copy/export works on local files**: If the page includes a copy button, test the `navigator.clipboard` path and include an `execCommand('copy')` fallback plus manual-selection fallback for `file://` pages.
- **Custom JavaScript validates before delivery**: If you wrote inline JS, syntax-check it before delivering (for example, by extracting the script and running `node --check`). Avoid accidental multiline string literals by using explicit `\n`, arrays + `join('\n')`, or `JSON.stringify(...)` when embedding structured data.
